...
class Admin::CategoriesControllerTest < ActionController::TestCase
  ...
  test 'get position' do
    ...
    get :position, params: { id: first_category.id, go: 'move_lower' }
    ...
    get :position, params: { id: second_category.id, go: 'move_higher' }
    assert assigns(:item).position.eql?(1)

    get :position, params: { id: first_category.id, go: 'move_to_bottom' }
    assert assigns(:item).position.eql?(2)

    get :position, params: { id: second_category.id, go: 'move_to_top' }
    assert assigns(:item).position.eql?(1)
  end

  test 'get position with not allowed go option' do
    category = categories(:default)
    get :position, params: { id: category.id, go: 'move_up' }
